...
extern "C" void LLVMInitializeCpu0Target() {
    ...
    RegisterTargetMachine<Cpu0elTargetMachine> X(getTheCpu0Target());
...
static std::string computeDataLayout(
    ...
    // 8 and 16 bits integers only need to have natural alignment, but try to align them to 32 bits. 64 bits integers have natural alignment.
...
static Reloc::Model getEffectiveRelocModel(
    ...
    if (!RM.has_value() || JIT) return Reloc::Static;
    return *RM;
}

// DataLayout --> Big-endian, 32-bit pointer/ABI/alignment
// The stack is always 8 byte aligned
// On function prologue, the stack is created by decrementing its pointer. Once decremented, all references are done with positive offset from the stack/frame pointer, using StackGrowsUp enables an easier handling.
Cpu0TargetMachine::Cpu0TargetMachine(
    const Target &T, const Triple &TT, StringRef CPU, StringRef FS,
    const TargetOptions &Options, std::optional<Reloc::Model> RM,
    std::optional<CodeModel::Model> CM, CodeGenOptLevel OL, bool JIT, bool isLittle
) : CodeGenTargetMachineImpl(
        T, computeDataLayout(TT, CPU, Options, isLittle), TT,
...
const Cpu0Subtarget *Cpu0TargetMachine::getSubtargetImpl(const Function &F) const {
    ...
    if (!I) {
        ...
        I = std::make_unique<Cpu0Subtarget>(
