...
static cl::opt<bool> EnableCpu0TailCalls(
    "enable-cpu0-tail-calls", cl::Hidden, 
    cl::desc("CPU0: Enable tail calls."), cl::init(false)
);

Cpu0SETargetLowering::Cpu0SETargetLowering(
    const TargetMachine &TM, const Cpu0Subtarget &STI
...
SDValue Cpu0SETargetLowering::LowerOperation(
    SDValue Op, SelectionDAG &DAG
) const {
    return Cpu0TargetLowering::LowerOperation(Op, DAG);
}

const Cpu0TargetLowering *llvm::createCpu0SETargetLowering(
    const TargetMachine &TM, const Cpu0Subtarget &STI
) {
