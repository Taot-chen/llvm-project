...
const ArrayRef<MCPhysReg> Cpu0ABIInfo::GetByValArgRegs() const {
    if (IsO32()) return makeArrayRef(O32IntRegs);
    if (IsS32()) return makeArrayRef(S32IntRegs);
...
Cpu0ABIInfo Cpu0ABIInfo::computeTargetABI() {
    ...
    if (EnableCpu0S32Calls) abi = ABI::S32;
    else abi = ABI::O32;
