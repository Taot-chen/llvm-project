...
) : Cpu0GenSubtargetInfo(TT, CPU, FS), IsLittle(little),
...
Cpu0Subtarget &Cpu0Subtarget::initializeSubtargetDependencies(
    ...
    if (TargetTriple.getArch() == Triple::cpu0
        ...
        errs() << "!!!Error, TargetTriple.getArch() = "
               << TargetTriple.getArch() << "CPU = " << CPU << "\n";
        exit(0);
    }

    if (CPU == "cpu032I") Cpu0ArchVersion = Cpu032I;
    else if (CPU == "cpu032II") Cpu0ArchVersion = Cpu032II;
